// ==UserScript==
// @name         HoboWars Wellness Helper - Additive (No Commas)
// @namespace    http://tampermonkey.net/
// @version      26.0
// @description  Hardcoded costs 1-100. Bank button ADDS to existing amount. No commas in output.
// @author       Gemini
// @match        *://www.hobowars.com/game/game.php?*cmd=wellness_clinic*
// @match        *://www.hobowars.com/game/game.php?*cmd=bank*
// @grant        none
// ==/UserScript==
...
(function() {
    ...
    const STORAGE_KEY = "hw_wellness_target_amt";
    ...
    // --- BANK PAGE ---
    if (url.includes('cmd=bank')) {
        const wellnessGoal = localStorage.getItem(STORAGE_KEY);
        ...
        if (withdrawInput && nativeWithdrawBtn) {
            const wellnessBtn = document.createElement('input');
            wellnessBtn.type = 'button';
            wellnessBtn.value = ` + Add Wellness ($${parseInt(wellnessGoal).toLocaleString()}) `;
            wellnessBtn.style.marginLeft = '10px';
            wellnessBtn.style.cursor = 'pointer';
            wellnessBtn.style.backgroundColor = '#e6f7ff';
            wellnessBtn.style.border = '1px solid #91d5ff';

            wellnessBtn.onclick = function() {
                // Read current input, strip commas, convert to integer
                let currentVal = parseInt(withdrawInput.value.replace(/,/g, '')) || 0;
                let goalVal = parseInt(wellnessGoal);

                // Result is a raw integer string (No commas)
            ...
            nativeWithdrawBtn.parentNode.insertBefore(wellnessBtn, nativeWithdrawBtn.nextSibling);
        }
    }
})();