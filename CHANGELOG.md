...
- Added custom 'ï¿½ï¿½eaveï¿½ï¿½' title display for SeventhHeaven.
...
- Implemented a combat window highlighter within the `HitlistHelper` that automatically shades rows an alerting light red if an opponent's level drastically falls outside the player's immediate attack limits (ï¿½ï¿½200 combat levels).
...
- Added an "Attack Range" checkbox filter to `ActiveListHelper` that instantly restricts the viewable opponent list exclusively to players falling within your immediate combat level range (ï¿½ï¿½200 levels of your current Hobo level). Filter persistently saves to local storage.
...
- Updated the success message text on the `FoodHelper` button to say "ï¿½S&  Updated Crap!" for better clarity.
...
- Added a `ï¿½x ï¿½ Save Repliers List` button to `MessageBoardHelper` explicitly for Gang Board posts (`cmd=gathering&do=vpost`), securely extracting and exporting a unique timestamped list of user names and IDs replying to the active topic locally.
